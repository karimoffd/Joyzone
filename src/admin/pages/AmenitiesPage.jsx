...
function GroupModal({ initial, onSave, onClose }) {
  ...
  const handleSubmit = async () => {
    ...
    const data = { ...form };
    if (!data.slug.startsWith("amenit")) {
      data.slug = "amenity-" + data.slug;
    }
    setSaving(true);
    try {
      if (isEdit) {
        await axios.put(`${API}/parameter-groups/${initial.id}/`, data, { headers: getHeaders() });
      } else {
        await axios.post(`${API}/parameter-groups/`, data, { headers: getHeaders() });
      }
      onSave();
      onClose();
    } catch (e) {
  ...
  return (
    <Modal title={isEdit ? "Редактировать группу" : "Новая группа параметров"} onClose={onClose}>
      <FormField label="Slug (URL ID)" hint="Только латинские буквы и дефисы">
        <input className="param-input" value={form.slug.replace(/^amenity-/, '')} onChange={e => set("slug", e.target.value.toLowerCase().replace(/[^a-z0-9-]/g, ""))} placeholder="basic" />
      </FormField>
...
function ParameterModal({ initial, groupId, groupName, onSave, onClose }) {
  ...
  return (
    <Modal title={isEdit ? "Редактировать удобство" : `Новое удобство → ${groupName}`} onClose={onClose}>
      <div style={{ display: "none" }}>
        {/* Hidden inputs to preserve structure if needed, or just removed entirely */}
      </div>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 12 }}>
        ...
        <FormField label="Иконка">
          <input className="param-input" value={form.icon} onChange={e => set("icon", e.target.value)} placeholder="Например: wifi, tv" />
        </FormField>
  ...
  return (
    <div>
      ...
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 24 }}>
        <div>
          <h2 style={{ margin: "0 0 2px", fontSize: 24, fontWeight: 800, color: "#1a1a2e" }}>Список удобств</h2>
          <p style={{ margin: "0 0 2px", fontSize: 13, color: "#64748b" }}>Создавайте группы и списки удобств для пространств</p>
